...
根目录 `README.md` 已精简为入口；**旧版根 README 中的文档链接均在本索引与各章节中保留**（见下）。本仓库整体大量借助 AI 辅助完成文档与代码，虽经人工审阅与测试，**仍可能有未覆盖路径、缺陷或文实不一致**，不构成任何担保；**生产环境请自行充分验证**。说明与实现不一致时以源码与测试为准，欢迎 Issue/PR。
